...
echo "开始安装mtr命令..."
which traceroute || sudo apt install -y tracerout
#yum clean all && yum makecache && yum install mtr -y
clear
echo -e "\n正在测试,请稍等..."
echo -e "——————————————————————————————\n"
for i in {0..8}; do
	sudo traceroute -n -T ${iplise[i]} > /tmp/traceroute_testlog
	...
	if [ $? == 0 ];then
		...
		if [ $? == 0 ];then
			...
			echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;34m电信163\033[0m"
				...
				grep -q "218\.105\."  /tmp/traceroute_testlog
				if [ $? == 0 ];then
				echo -e "目标:${iplocal[i]}[${iplise[i]}]\t回程线路:\033[1;35m联通9929\033[0m"
				else
					grep -q "219\.158\."  /tmp/traceroute_testlog
					if [ $? == 0 ];then
						grep -q "219\.158\.113\." /tmp/traceroute_testlog
						if [ $? == 0 ];then
